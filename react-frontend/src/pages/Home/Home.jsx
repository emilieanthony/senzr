...
const calculateTemperatureThreshold = (temperature) => {
  if (
    temperature >= COLD_INDOOR_TEMPERATURE_DEGREES &&
    temperature <= LOW_INDOOR_TEMPERATURE_DEGREES
  ) {
    return "medium";
  }
  if (
    temperature >= HIGH_INDOOR_TEMPERATURE_DEGREES &&
    temperature <= WARM_INDOOR_TEMPERATURE_DEGREES
  ) {
    return "medium";
  }
...
const calculateHumidityThreshold = (humidity) => {
  if (
    humidity > VERY_LOW_INDOOR_HUMIDITY_PERCENT &&
    humidity <= LOW_INDOOR_HUMIDITY_PERCENT
  ) {
    return "medium";
  }
  if (
    humidity > HIGH_INDOOR_HUMIDITY_PERCENT &&
    humidity <= VERY_HIGH_INDOOR_HUMIDITY_PERCENT
  ) {
    return "medium";
  }
